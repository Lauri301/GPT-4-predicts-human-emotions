# Boxplot of emotion-specific distances between human and GPT-4 ratings for video dataset 2
#
...
human <- read_csv('path/tettamanti_project/raitings/data/human_data/average_ratings/combined_averages_without_t4.csv')
gpt <- read_csv('path/tettamanti_project/raitings/data/gpt_4-1_data/average/all_averages/output_average_10_files_1_2_3_4_5_6_7_8_9_10.csv')
...
names(human)[names(human) == "Row"] <- "videoNames"
...
gpt <- gpt %>% arrange(videoNames)
human <- human %>% arrange(videoNames)

# Remove non-numeric columns (VideoName) for numeric operations
dimensio_human <- human %>% select(-videoNames)
dimensio_gpt <- gpt %>% select(-videoNames)
...
dist <- data.frame(videoNames = human$videoNames)
...
dist_long <- dist %>%
  pivot_longer(cols = -videoNames, names_to = "emotion", values_to = "distance")
...
# Drawing boxplot
pdf('path/TET_boxplot_confidence_interval_and_median.pdf', width = 15, height = 6)
