# Collecting multiple independent evaluations for same stimuli increases the accuracy of GPT-4 ratings (image dataset)
#
...
human_data_long <- human_data %>%
  pivot_longer(cols = -imageNames, names_to = "emotion", values_to = "mean_human_rating")
...
for (file in gpt_files) {
  ...
  # Remove the '.jpg' file extension from image names to match with human data identifiers
  gpt_data <- gpt_data %>%
    mutate(imageNames = str_replace(imageNames, "\\.jpg$", ""))

  # Match GPT video rows to human data by imageNames
  gpt_data <- gpt_data[match(human_data$imageNames, gpt_data$imageNames), ]
  ...
  gpt_data_long <- gpt_data %>%
    pivot_longer(cols = -imageNames, names_to = "emotion", values_to = "mean_gpt_rating")
  
  # Merge GPT and human long-form data by video and emotion
  combined_long <- left_join(human_data_long, gpt_data_long, by = c('imageNames', 'emotion'))
