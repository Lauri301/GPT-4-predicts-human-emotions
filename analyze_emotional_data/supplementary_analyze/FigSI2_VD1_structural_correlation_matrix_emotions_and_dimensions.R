# The similarity of the emotion rating structures for video dataset 1
#
...
gpt_data <- read.csv('path/data/VD1/ratings/data/gpt-4-1_data/average/all_averages/output_average_10_files_1_2_3_4_5_6_7_8_9_10.csv')
...
human_emotions_cor_ordered_1 <- human_emotions_cor[colnames(human_emotions_cor_ordered), colnames(human_emotions_cor_ordered)]
gpt_emotions_cor_ordered_1 <- gpt_emotions_cor[colnames(human_emotions_cor_ordered_1), colnames(human_emotions_cor_ordered_1)]

# Reorder dimensions correlation matrix
human_dimensions_cor_ordered_1 <- human_dimensions_cor[colnames(human_dimensions_cor_ordered), colnames(human_dimensions_cor_ordered)]
gpt_dimensions_cor_ordered_1 <- gpt_dimensions_cor[colnames(human_dimensions_cor_ordered_1), colnames(human_dimensions_cor_ordered_1)]
...
pdf("path/VD1_gpt_emotions_structural_correlation_matrix.pdf", width = 15, height = 15)

# Plot GPT emotions matrix (lower triangle only)
corrplot(gpt_emotions_cor_ordered_1, method = 'color', tl.col = 'black', type = 'lower', col = colorRampPalette(c("#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#FDDBC7", "#F4A582", "#D6604D", "#B2182B"))(20))
dev.off()

# Save GPT dimensions correlation matrix as PDF
pdf("path/VD1_gpt_dimensions_structural_correlation_matrix.pdf", width = 15, height = 15)

# Plot GPT dimensions matrix (lower triangle only)
corrplot(gpt_dimensions_cor_ordered_1, method = 'color', tl.col = 'black', type = 'lower', col = colorRampPalette(c("#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#FDDBC7", "#F4A582", "#D6604D", "#B2182B"))(20))
dev.off()

#### HUMAN ####

# Save human emotions correlation matrix as PDF
pdf("path/VD1_human_emotions_structural_correlation_matrix.pdf", width = 15, height = 15)

# Plot human emotions matrix (upper triangle only)
corrplot(human_emotions_cor_ordered_1, method = 'color', tl.col = 'black', type = 'upper', col = colorRampPalette(c("#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#FDDBC7", "#F4A582", "#D6604D", "#B2182B"))(20))
dev.off()

# Save human dimensions correlation matrix as PDF
pdf("path/VD1_human_dimensions_structural_correlation_matrix.pdf", width = 15, height = 15)

# Plot human dimensions matrix (upper triangle only)
corrplot(human_dimensions_cor_ordered_1, method = 'color', tl.col = 'black', type = 'upper', col = colorRampPalette(c("#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#FDDBC7", "#F4A582", "#D6604D", "#B2182B"))(20))
dev.off()
...
gpt_emotions_vector <- as.vector(gpt_emotions_cor_ordered_1[lower.tri(gpt_emotions_cor_ordered_1)])
gpt_dimensions_vector <- as.vector(gpt_dimensions_cor_ordered_1[lower.tri(gpt_dimensions_cor_ordered_1)])

human_emotions_vector <- as.vector(human_emotions_cor_ordered_1[lower.tri(human_emotions_cor_ordered_1)])
human_dimensions_vector <- as.vector(human_dimensions_cor_ordered_1[lower.tri(human_dimensions_cor_ordered_1)])
...
p_value_emotions <- mantel.test(gpt_emotions_cor_ordered_1, human_emotions_cor_ordered_1, nperm = 1000000, graph = T, alternative = "greater")
p_value_dimensions <- mantel.test(gpt_dimensions_cor_ordered_1, human_dimensions_cor_ordered_1, nperm = 1000000, graph = T, alternative = "greater")
