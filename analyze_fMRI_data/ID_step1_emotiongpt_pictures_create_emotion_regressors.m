%% EmotionGPT: Build GPT and human regressors for the megafmri data

% Severi Santavirta 5.3.2025, Lauri Suominen 21.8.2025

%% INPUT

datapath_gpt = 'path/naps/output_average_10_files_1_2_3_4_5_6_7_8_9_10.csv';
datapath_human = 'path/naps/human_average_ratings_gpt-4-1.csv';
datapath_log = 'path/presentation_log_files';
output = 'path/naps/regressors';
...
for sub = 1:size(subjects,1)
    ...
    % Build regressors
    [Rgpt,TSgpt,tRgpt,tTSgpt] = pictures_create_regressors(feature_ratings_gpt,feature_clips,presented_video_order,presented_video_onsets,presented_video_durs,num_vols,tr);
    [Rhuman,TShuman,tRhuman,tTShuman] = pictures_create_regressors(feature_ratings_human,feature_clips,presented_video_order,presented_video_onsets,presented_video_durs,num_vols,tr);

    % Save the data
    save(sprintf('%s/gpt_reg/pictures_%s_gpt_regressors.mat',output,subjects{sub}),'Rgpt','TSgpt','tRgpt','tTSgpt','features_gpt');
    save(sprintf('%s/human_reg/pictures_%s_human_regressors.mat',output,subjects{sub}),'Rhuman','TShuman','tRhuman','tTShuman','features_human');
...
function [R,TS,tR,tTS] = pictures_create_regressors(feature_ratings,feature_clips,presented_video_order,presented_video_onsets,presented_video_durs,num_vols,tr)
