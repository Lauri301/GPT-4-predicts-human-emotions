...
% Batch the first level analysis over emotion features
%
...
% Directories
preproc_dir = 'path/naps/preprocessed_3mm';
regressor_dir = 'path/naps/regressors/human_reg';
first_level_dir = 'path/naps/fmri_analysis/first_level/human';
brainmask = 'path/megafmri_localizer_gm_mask_3mm.nii';
...
for j = 1:length(features_human)-1
    ...
    tmp_folder = sprintf('path/naps/tmp/%s',feature);
    ...
    for i = 1:size(subjects,1)
        ...
        reg_file = sprintf('%s/pictures_%s_human_regressors.mat',regressor_dir,sub);
        load(reg_file); % This loads 'R' and 'features'
        
        % Extract the relevant column from 'R'
        R = Rhuman(:,[j,end]);
        R = zscore(R);
        names = {feature;features_human{end}}'; % Use the feature name as the name
