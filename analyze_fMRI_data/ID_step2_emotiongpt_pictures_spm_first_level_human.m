%% EmotionGPT: First level analyses of the localizer fMRI data with human derived regressors
% Batch the first level analysis over social features
%
% Severi Santavirta 26.3.2025, Lauri Suominen 21.8.2025
...
% Directories
preproc_dir = 'path/fmri_analysis/ID_fmri_gpt-4-1/preProg';
regressor_dir = 'path/fmri_analysis/ID_fmri_gpt-4-1/regressors/human_reg';
first_level_dir = 'path/fmri_analysis/ID_fmri_gpt-4-1/first_level_human';
brainmask = 'path/fmri_analysis/localizer/megafmri_localizer_gm_mask_3mm.nii';
...
bad_preproc = {'sub-062','sub-100','sub-013','sub-007'}; % These subjects had glitches after preprocessing which affected their first level maps.
...
files = find_files(regressor_dir,'*.mat');
load(files{1});
for j = 1:length(features_human)-1
    feature = features_human{j}; % Extract the string from the cell
    
    tmp_folder = sprintf('path/fmri_analysis/ID_fmri_gpt-4-1/regressors/feature_models_human/%s',feature);
    ...
    for i = 1:size(subjects,1)
        ...
        load(reg_file); % This loads 'R' and 'features_human'
        
        % Extract the relevant column from 'R'
        R = Rhuman(:,[j,end]);
        R = zscore(R); % Standardize
    ...
    parfor i = 1:size(subjects,1)
        ...
        if(~exist(con_file,'file'))
            ...
            model_file = sprintf('%s/%s_model.mat',tmp_folder,sub);
