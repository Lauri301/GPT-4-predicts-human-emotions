%% EmotionGPT: First level analyses of the localizer fMRI data with lowlevel regressors
...
regressor_dir = 'path/naps/lowlevel/pca_regressors';
first_level_dir = 'path/naps/fmri_analysis/first_level/lowlevel_confounds';
...
for j = 1:length(features)-1
    ...
    tmp_folder = sprintf('path/naps/tmp3/%s',feature);
    ...
    for i = 1:size(subjects,1)
        ...
        reg_file = sprintf('%s/pictures_%s_lowlevel_regressors.mat',regressor_dir,sub);
        load(reg_file); % This loads 'R' and 'features'
        
        % Extract the relevant column from 'R'
        R = R(:,[j,end]);
        R = zscore(R);
        names = {feature;features{end}}'; % Use the feature name as the name
