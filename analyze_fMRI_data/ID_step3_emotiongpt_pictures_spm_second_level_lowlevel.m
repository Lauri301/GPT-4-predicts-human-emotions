%% EmotionGPT: Second level analyses of pictures fMRI data with lowlevel regressors
% Batch the second level analysis over eatch emotion
%
% Severi Santavirta & Wuhang Wu, Lauri Suominen 1.2.2026

% Define the directories
base_indir = 'path/naps/fmri_analysis/first_level/lowlevel_confounds';
base_output = 'path/naps/fmri_analysis/second_level/lowlevel_confounds';
mask = 'path/megafmri_localizer_gm_mask_3mm.nii';
...
bad_preproc = {'sub-062','sub-007','sub-013','sub-100'}; %%%%%% These subjects had glitches after preprocessing which affected their first level maps.
...
for j = 1:length(created_folders)
    ...
    out = estimate_second_level_model(feature_output);
    out = define_contrasts_main_effect_models(feature_output, sprintf('main_%s', feature));
end

%% Functions

function out = define_second_level_main_effects_model(contrast_image,subjects,brainmask,indir,outdir)
% Anlayze main effects without covariates based on input contrasts and
% subjects
