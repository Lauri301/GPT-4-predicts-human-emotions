%% EmotionGPT: First level analyses of the localizer fMRI data with lowlevel (audiovisual confounds) regressors
% Batch the first level analysis over lowlevel features
%
% Severi Santavirta 4.1.2026, Lauri Suominen 21.8.2025
...
% Directories
preproc_dir = 'path/fmri_analysis/localizer/preprocessed_3mm';
lowlevel_dir = 'path/pca_regressors';
first_level_dir = 'path/fmri_analysis/localizer/first_level/lowlevel_confounds';
...
files = find_files(lowlevel_dir,'*.mat');
load(files{1});

% Select only the first eight PCs
for j = 15:22
    feature = cats_lowlevel{j}; % Extract the string from the cell
    
    tmp_folder = sprintf('path/tmp/%s',feature);
    ...
    for i = 1:size(subjects,1)
        ...
        load(sprintf('%s/megafmri-localizer-%s_lowlevel_regressors.mat', lowlevel_dir, sub));
    ...
    parfor i = 1:size(subjects,1)
        ...
        if(~exist(con_file,'file'))
            ...
            scans = cellstr(spm_select('ExtFPList',preproc_dir,sprintf('%s_task-localizer_space-MNI152NLin2009cAsym_desc-smoothAROMAnonaggr_bold',sub),1:467));
